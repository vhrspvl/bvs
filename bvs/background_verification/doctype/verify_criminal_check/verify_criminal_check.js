...
frappe.ui.form.on("Verify Criminal Check", {
    after_save: function (frm) {
        ...
        if ((frm.doc.status == "Execution Completed") && ((frm.doc.result == "Positive") || (frm.doc.result == "Negative") || (frm.doc.result == "Amber") || (frm.doc.result == "Insufficient"))) {
            frappe.set_route("List", "Verify Criminal Check");
        }
    },
    onload: function (frm) {
        frappe.call({
            "method": "bvs.background_verification.doctype.verify_criminal_check.verify_criminal_check.get_check",
            args: {
            ...
            callback: function (r) {
                $.each(r.message, function (i, d) {
                    if (r.message) {
                        frm.set_value("criminal_check_id", d.name);
                    }
                });
            }
    ...
    refresh: function (frm) {
        ...
        if (frm.doc.fields != "Updated") {
            frappe.call({
                ...
                args: {
                    doctype: "Criminal Check",
                    filters: {
                        "name": frm.doc.criminal_check_id
                    }
                },
                callback: function (r) {
                    if (r.message) {
                        frm.set_value('ver_address_line_1', r.message.address_line1);
                        frm.set_value('ver_address_line_2', r.message.address_line2);
                        frm.set_value('ver_address_line_3', r.message.address_line3);
        ...
        if ((frm.doc.allocated_for == "QC Pending") || (frm.doc.status == "QC Completed")) {
            frm.set_df_property('ver_address_line_1', 'read_only', 0);
            frm.set_df_property('ver_address_line_2', 'read_only', 0);
            frm.set_df_property('ver_address_line_3', 'read_only', 0);
