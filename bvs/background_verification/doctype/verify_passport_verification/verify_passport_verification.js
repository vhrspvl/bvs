...
frappe.ui.form.on("Verify Passport Verification", {
		...
		frappe.call({
			...
			callback: function(r){
				$.each(r.message, function(i, d) {
					if(r.message){
						frm.set_value("passport_verification_id", d.name);
					}
				});
			}
		...
		if(frm.doc.applicant_id){
			frappe.call({
				...
				args:{
					...
					filters:{
						"name": frm.doc.passport_verification_id
					 }
				},
				callback: function(r){
				   if(r.message){
					frm.set_value('ver_passport_number', r.message.passport_number);
					frm.set_value('ver_sur_name', r.message.sur_name);
					frm.set_value('ver_given_name', r.message.given_name);
					frm.set_value('ver_place_of_birth', r.message.place_of_birth);
					...
					frm.set_value('ver_date_of_expiry', r.message.date_of_expiry);
					frm.set_value('ver_nationality', r.message.nationality);
