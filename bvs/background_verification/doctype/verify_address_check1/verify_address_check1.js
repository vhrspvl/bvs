...
frappe.ui.form.on("Verify Address Check1", {
    after_save: function (frm) {
        ...
        if ((frm.doc.status == "Execution Completed") && ((frm.doc.result == "Positive") || (frm.doc.result == "Negative") || (frm.doc.result == "Amber") || (frm.doc.result == "Insufficient"))) {
            frappe.set_route("List", "Verify Address Check1");
        }
        if (frm.doc.tat) {
    ...
    onload: function (frm) {
        frappe.call({
            "method": "bvs.background_verification.doctype.verify_address_check1.verify_address_check1.get_check",
            args: {
            ...
            callback: function (r) {
                $.each(r.message, function (i, d) {
                    if (r.message) {
                        frm.set_value("address_check1_id", d.name);
                    }
                });
            }
    ...
    refresh: function (frm) {
        ...
        if (frm.doc.fields != "Updated") {
            frappe.call({
                ...
                args: {
                    doctype: "Address Check1",
                    filters: {
                        "name": frm.doc.address_check1_id
                    }
                },
                callback: function (r) {
                    if (r.message) {
