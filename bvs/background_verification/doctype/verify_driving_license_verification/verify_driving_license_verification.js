...
frappe.ui.form.on("Verify Driving License Verification", {
	...
	onload:function(frm){
		...
		frappe.call({
			"method": "bvs.background_verification.doctype.verify_driving_license_verification.verify_driving_license_verification.get_check",
			args: {
			...
			callback: function(r){
				$.each(r.message, function(i, d) {
					if(r.message){
						frm.set_value("driving_license_verification_id", d.name);
					}
				});
			}
	...
	refresh: function(frm){
		...
		if(frm.doc.applicant_id){
			frappe.call({
				...
				args:{
					doctype: "Driving License Verification",
					filters:{
						"name": frm.doc.driving_license_verification_id
					 }
				},
				callback: function(r){
				   if(r.message){
					frm.set_value('ver_driving_license_number', r.message.driving_license);
					frm.set_value('ver_address', r.message.address);
					frm.set_value('ver_dob', r.message.dob);
					frm.set_value('ver_valid_till', r.message.valid_till);					 
				   }
				}
			})
		}
