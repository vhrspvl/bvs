...
frappe.ui.form.on("Applicant", {
    ...
    id_check: function(frm){
        if(frm.doc.id_check_status != "Updated"){
            var dialog = new frappe.ui.Dialog({
                title: __("Select ID"),
                fields: [
                    {fieldtype: "Check", fieldname: "aadhar_card", label: __("Aadhar Card"), reqd: 0},
                    {fieldtype: "Check", fieldname: "pan_card", label: __("Pan Card"), reqd: 0},
                    {fieldtype: "Check", fieldname: "passport", label: __("Passport"), reqd: 0},
                    {fieldtype: "Check", fieldname: "ration_card", label: __("Ration Card"), reqd: 0},
                    {fieldtype: "Check", fieldname: "voters_id", label: __("Voter's ID"), reqd: 0},
                    {fieldtype: "Check", fieldname: "driving_license", label: __("Driving License"), reqd: 0}
                ]
            });
            dialog.show();	
            frm.set_value("id_check_status", "Updated");		
        } else {
            frappe.call({
                "method":"frappe.client.get",
                args: {
                ...
                callback: function(r){
                    if(r.message){
                        var dialog = new frappe.ui.Dialog({
                            title: __("Select ID"),
                            fields: [
                                {fieldtype: "Check", fieldname: "aadhar_card", label: __("Aadhar Card"), default: r.message.aadhar_card_verification, reqd: 0},
                                {fieldtype: "Check", fieldname: "pan_card", label: __("Pan Card"), default: r.message.pan_verification, reqd: 0},
                                {fieldtype: "Check", fieldname: "passport", label: __("Passport"), default: r.message.passport_verification, reqd: 0},
                                {fieldtype: "Check", fieldname: "ration_card", label: __("Ration Card"), default: r.message.ration_card_verification, reqd: 0},
                                {fieldtype: "Check", fieldname: "voters_id", label: __("Voter's ID"), default: r.message.voters_id_verification, reqd: 0},
                                {fieldtype: "Check", fieldname: "driving_license", label: __("Driving License"), default: r.message.driving_license_verification, reqd: 0}
                            ]
                        });
                        dialog.show();	
                    }
                    dialog.set_primary_action(__("Update"), function() {
                        ...
                        if(!args) return;
                            ...
                            frappe.call({
                                "method":"bvs.background_verification.doctype.applicant.applicant.update_checks",
                                args: {
                                    "checks_group": frm.doc.checks_group,
                                    "aadhar_card": args.aadhar_card,
                                    "pan_card": args.pan_card,
                                    "passport": args.passport,
                                    "ration_card": args.ration_card,
                                    "voters_id": args.voters_id,
                                    "driving_license": args.driving_license	
                                },
                                callback: function(r){
                                    if(r.message){
                                        dialog.hide();
                                        ...
                                        if(frm.doc.ref_id){
                                            ...
                                            frappe.msgprint("Please save the document before modifing.")
                                        }                   
                                    }
                                }
                            })
