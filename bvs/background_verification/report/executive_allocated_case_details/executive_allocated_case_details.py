...
def execute(filters=None):
    ...
    for a in app:
        row = [a.in_date,a.in_date,a.customer,a.name,a.candidate_name,a.client_employee_code,a.executive,a.status,a.remarks,a.end_date]
        data.append(row)
        if not a.end_date:
...
def applicants(filters):
    applicant = frappe.db.sql(
        """select `tabData Entry Allocation`.in_date,`tabData Entry Allocation`.customer,`tabData Entry Allocation`.executive,`tabApplicant`.name,`tabApplicant`.candidate_name,`tabApplicant`.client_employee_code,`tabApplicant`.status,`tabApplicant`.end_date,`tabApplicant`.tat_stop_date,`tabApplicant`.tat_resume_date,`tabApplicant`.remarks  from `tabApplicant` 
        LEFT JOIN `tabData Entry Allocation` ON `tabApplicant`.data_entry_allocation_id = `tabData Entry Allocation`.name
        where `tabApplicant`.in_date between %(start_date)s and %(end_date)s order by `tabApplicant`.in_date""",{
