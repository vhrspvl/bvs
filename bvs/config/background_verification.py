...
def get_data():
    ...
    bvs_manager =  frappe.get_doc("User", frappe.session.user).get("roles",{"role": "BVS Manager" })
    bvs_verifier = frappe.get_doc("User", frappe.session.user).get("roles",{"role": "BVS Verifier" })
    bvs_deo = frappe.get_doc("User", frappe.session.user).get("roles",{"role": "BVS DEO" })
    if system_manager:
        return [
        {
